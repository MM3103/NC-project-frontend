...
const CreateNewOrder = () => {
    ...
    if (streets.length !== undefined) {
        for (let i = 0; i < streets.length; i++) {
            if (streets[i].city.name === cityName) {
                streetsOnCity[0]=''
                streetsOnCity[streetsOnCity.length ] = streets[i].name
            }
        }
        ...
        if (streets.length !== undefined) {
            for (let i = 0; i < streets.length; i++) {
                if ((nameCity === streets[i].city.name) && (streetName === streets[i].name)) {
                    handleSubmit(streets[i].city.id,nameCity,streets[i].id,streetName)
                }
            }
    ...
    function handleSubmit(cityId, nameCity,streetID,streetName) {
        const city = {
            id: cityId,
            name: nameCity
        }
        ...
        console.log(selfInstallation)
        const response = axios.post('http://localhost:8484/order', {typeOrder, city, street, house, flat, selfInstallation},
            {
                headers: {
